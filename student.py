...
class Student:
    ...
    # This part is image labels setting start 
        ...
        dep_combo["values"]=("Select Department","ISE","CSE","ECE","EEE","ME","CV","AI")
        dep_combo.current(0)
        ...
        cou_combo["values"]=("Select Course","BE","ME","BCA","MCA","MBA")
        cou_combo.current(0)
        ...
        #Class Didvision
        student_div_label = Label(class_Student_frame,text="Section:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        ...
        #USN
        student_roll_label = Label(class_Student_frame,text="USN:",font=("verdana",12,"bold"),fg="navyblue",bg="white")
        ...
        #update photo button
        update_photo_btn=Button(btn_frame,command=self.update_dataset,text="Take Photo",width=9,font=("verdana",12,"bold"),fg="white",bg="navyblue")
        ...
        search_combo["values"]=("Select","USN")
...
# ==================================Generate Data set take image=========================
        ...
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_div.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="" or self.var_teacher.get()=="":
            ...
            try:
                ...
                while True:
                    ...
                    if cv2.waitKey(1)==13 or int(img_id)==170:
                        break
                cap.release()
    ...
    def update_dataset(self):
        try:
            ...
            studid = self.student_table.item(curItem)['values'][0]
        except:
            messagebox.showerror("Error","Select a student first!",parent=self.root)
            return
        if self.var_dep.get()=="Select Department" or self.var_course.get=="Select Course" or self.var_year.get()=="Select Year" or self.var_semester.get()=="Select Semester" or self.var_std_id.get()=="" or self.var_std_name.get()=="" or self.var_div.get()=="" or self.var_roll.get()=="" or self.var_gender.get()=="" or self.var_dob.get()=="" or self.var_email.get()=="" or self.var_mob.get()=="" or self.var_address.get()=="" or self.var_teacher.get()=="":
            messagebox.showerror("Error","Please Fill All Fields are Required!",parent=self.root)
        else:
            try:
                ...
                while True:
                    ...
                    if face_croped(my_frame) is not None:
                        ...
                        face=cv2.resize(face_croped(my_frame),(200,200))
                        face=cv2.cvtColor(face,cv2.COLOR_BGR2GRAY)
                        file_path="data_img/stdudent."+str(studid)+"."+str(img_id)+".jpg"
                        cv2.imwrite(file_path,face)
                        cv2.putText(face,str(img_id),(50,50),cv2.FONT_HERSHEY_COMPLEX,2,(0,255,0),2)        
                        cv2.imshow("Capture Images",face)
